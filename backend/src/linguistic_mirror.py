...
def detect_linguistic_style(accumulated_text: str) -> dict:
    ...
    # Detect non-Latin scripts (Sinhala, Tamil, Telugu, Devanagari, etc.)
    non_ascii_chars = sum(1 for c in raw_text if ord(c) > 127)
    ascii_chars = max(len(raw_text) - non_ascii_chars, 1)
    non_ascii_ratio = non_ascii_chars / max(len(raw_text), 1)

    if hindi_score >= 3:
        language = "hinglish"
    elif non_ascii_ratio > 0.3:
        language = "non_english"  # Non-Latin script detected (Sinhala, Tamil, etc.)
    else:
        language = "english"
...
def compose_mirror_instruction(style: dict) -> str:
    ...
    elif language == "non_english":
        parts.append(
            "Language: Customer may not be fluent in English — they used a non-English script. "
            "Speak simple, clear English. Short sentences. Avoid idioms and complex vocabulary."
        )
    else:
